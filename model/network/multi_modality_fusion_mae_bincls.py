...
class MultimodalBertModel(nn.Module):
    ...
    def forward(self, audio_input, text_input, bin_cls=False):
        """
        ...
        # 分类器输出
        logits = self.binary_classifier(cls_output) if bin_cls else self.classifier(cls_output)

        return logits, cls_output, seq_output # 返回分类logits、None(原binary_logits位置)、融合特征
