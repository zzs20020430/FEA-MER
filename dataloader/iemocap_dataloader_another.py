...
def data_filter(data:list,args):
    ...
    filter_aug=lambda item:{
                    'text':item['text_aug'],            #text
                    # 'waveform':waveform.numpy().squeeze(0),          #audio
                    'audio':item['audio_aug'],            #audio
                    'label':item['label'],      #label
                    'duration':item['duration'],                             #seconds
                    'filename':item['filename'],
                    'sample_rate':item['sample_rate']
    ...
    return [filter_raw(item) for item in filtered_data]+[filter_aug(item) for item in filtered_data if item.get('text_aug',None) is not None]
